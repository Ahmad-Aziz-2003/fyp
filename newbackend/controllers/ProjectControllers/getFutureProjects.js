 ...
 */
async function getFutureProjects(req, res) {
...
try {
  ...
  if (completedProjects.length === 0) {
    return res.status(404).json({ message: "No future projects found for the given NGO ID." });
  }
...
module.exports = { getFutureProjects };


//   try {
//     const projectsRef = db.ref("projects");
//     const snapshot = await projectsRef.orderByChild("status").equalTo("future").once("value");

//     if (!snapshot.exists()) {
//       return res.status(404).json({ message: "No future projects found." });
//     }
